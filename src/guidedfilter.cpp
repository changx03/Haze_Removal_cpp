...
cv::Mat GuidedFilterImpl::filter(const cv::Mat& p, int depth)
{
    ...
    cv::Mat result;
    if(p.channels() == 1) { result = filterSingleChannel(p2); }
    else
    {
...
GuidedFilterMono::GuidedFilterMono(const cv::Mat& origI, int r, double eps)
    : r(r),
      eps(eps)
{
...
GuidedFilterColor::GuidedFilterColor(const cv::Mat& origI, int r, double eps)
    : r(r),
      eps(eps)
{
    cv::Mat I;
